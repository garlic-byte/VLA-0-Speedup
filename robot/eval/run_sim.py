...
from robot.eval.sim.wrapper.multistep_wrapper import MultiStepWrapper
...
# Filter ONLY the specific UserWarning about obs type
warnings.filterwarnings('ignore', category=UserWarning, message=r".*The obs returned by the.*")
warnings.filterwarnings('ignore', category=UserWarning, message=r".*Casting input x to numpy array..*")
...
def get_libero_env_fn(
    ...
    def env_fn():
        from robot.eval.sim.libero_env import register_libero_envs
...
def create_eval_env(
    ...
    if wrapper_configs.video.video_dir is not None:
        from robot.eval.sim.wrapper.video_recording_wrapper import (
            VideoRecorder,
...
def run_rollout_gymnasium_policy(
    ...
    Args:
        env_name: Name of the gymnasium environment to use
        policy: Function that creates a policy instance
...
def run_gr00t_sim_policy(
    ...
    video_dir = f"/home/wsj/Desktop/code/VLA/robot/outputs/video/sim_eval_videos_{env_name}_ac{n_action_steps}_{uuid.uuid4()}"
    wrapper_configs = WrapperConfigs(
        video=VideoConfig(
            video_dir=video_dir,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy_client_host", type=str, default="localhost")
    parser.add_argument("--policy_client_port", type=int, default=8000)
    parser.add_argument(
        "--env_name",
        type=str,
        default="libero_sim/pick_up_the_alphabet_soup_and_place_it_in_the_basket",
    )
