...
class HDF5Processor:
    ...
    def _process_libero_demo_format(self, file: h5py.File, dataset: LeRobotDataset, task_name: str,
    ...
    def _extract_libero_demo_frames(self, file: h5py.File, task_name: str, file_path: Optional[Path] = None) -> List[
        List[Dict]]:
    ...
    def _extract_direct_episode_frames(self, file: h5py.File, task_name: str, file_path: Optional[Path] = None) -> List[
        List[Dict]]:
...
class RLDSProcessor:
    ...
    def __init__(self):
        if not HAS_TF:
            raise ImportError(
                "tensorflow_datasets是RLDS处理所必需的，请运行: pip install tensorflow tensorflow_datasets")
...
class UnifiedConverter:
    ...
    def _process_hdf5_data(self, processor: Union[HDF5Processor, RLDSProcessor], dataset: LeRobotDataset,
                           data_path: Path, task_name: str):
        """使用多线程处理HDF5数据"""
        # 确保是HDF5Processor
    ...
    def _process_episodes_parallel(self, processor: HDF5Processor, dataset: LeRobotDataset, episodes: List[Path],
                                   task_name: str):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            # 收集结果
            for future in tqdm(concurrent.futures.as_completed(future_to_episode), total=len(episodes),
                               desc="处理Episodes"):
